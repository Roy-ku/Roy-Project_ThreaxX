...
{
  if (GPIO_Pin == GTP_INT_GPIO_PIN) //确保是否产生了EXTI Line中断
  {
    //GTP_TouchProcess();
    Touch_event = 1;
  }
...
{
  uint8_t status;
  status = bsp_I2Cx_bus_Master_Read(&hi2c2, ClientAddr, pBuffer, NumByteToRead, GTP_FLAG_TIMEOUT);
  if (status != HAL_OK)
  {
...
{
  uint8_t status;
  status = bsp_I2Cx_bus_Master_Write(&hi2c2, ClientAddr, pBuffer, NumByteToWrite, GTP_FLAG_TIMEOUT);
